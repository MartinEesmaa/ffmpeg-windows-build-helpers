...
build_cmake() {
  download_and_unpack_file https://cmake.org/files/v3.19/cmake-3.19.2.tar.gz
  cd cmake-3.19.2
...
build_libmpg123() {
  download_and_unpack_file https://sourceforge.net/projects/mpg123/files/mpg123/1.26.4/mpg123-1.26.4.tar.bz2
  cd mpg123-1.26.4
    if [[ ! -f Makefile.in.bak ]]; then # Library only
...
build_curl() {
  download_and_unpack_file https://curl.haxx.se/download/curl-7.72.0.tar.bz2
  if [ "$1" = "openssl" ]; then # Compile Curl with OpenSSL for hlsdl.
    ...
    mkdir -p $redist_dir
    archive="$redist_dir/curl-7.72.0_mbedtls_zlib-win32-static-xpmod-sse"
