...
install_cross_compiler() {
  ...
  if [[ -f $win32_gcc ]]; then
    ...
    cd cross_compilers
      ...
      local zeranoe_script_name=mingw-w64-build-r25 # https://files.1f0.de/mingw/scripts/
...
build_mbedtls() {
  download_and_unpack_file https://github.com/ARMmbed/mbedtls/archive/mbedtls-2.16.0.tar.gz mbedtls-mbedtls-2.16.0
  mkdir -p mbedtls-mbedtls-2.16.0/build_dir
  cd mbedtls-mbedtls-2.16.0/build_dir # Out-of-source build.
...
build_libopenmpt() {
  download_and_unpack_file https://lib.openmpt.org/files/libopenmpt/src/libopenmpt-0.3.11+release.autotools.tar.gz
